        ...
        for (size_t x1 = 0; x1 < grid[y1].size(); ++x1) {
            for (size_t y2 = 0; y2 < grid.size(); ++y2) {
                for (size_t x2 = 0; x2 < grid[y2].size(); ++x2) {
                    ...
                    size_t antinode_y = y2;
                    size_t antinode_x = x2;
                    
                    const size_t antinode_dy = y2 - y1;
                    const size_t antinode_dx = x2 - x1;

                    while (
                        (antinode_y >= 0) &&
                        (antinode_y < grid.size()) &&
                        (antinode_x >= 0) && 
                        (antinode_x < grid[antinode_y].size())
                    ) {
                        if (!antinodes[antinode_y][antinode_x]) {
                            antinodes[antinode_y][antinode_x] = true;
                            ++number_antinodes;
                        }
                        antinode_y += antinode_dy;
                        antinode_x += antinode_dx;
                    }
                }
            }
        }
